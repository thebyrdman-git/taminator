...
**For the fewest issues:** Use the **packaged desktop app** from [GitLab releases](https://gitlab.cee.redhat.com/jbyrd/taminator/-/releases). No Python or `pip install` required; the app includes everything it needs.
...
- **From repo:** `git clone https://gitlab.cee.redhat.com/jbyrd/taminator.git ~/taminator` (for CLI or development). Run `pip install -r requirements.txt` in the repo before using CLI or running the UI from source.
...
- **Customer Portal token** (optional) — For **portal group posting** and **case discovery**. From access.redhat.com/management/api → Generate Token. One token covers both.
...
### Building the desktop app (packaged DMG/AppImage)

The packaged app bundles a Python environment so end users do not need to install Python or run `pip install`. To build locally from the repo, create the bundle first, then run the Electron build:

```bash
cd taminator/taminator
./scripts/bundle-python-deps.sh
cd gui && npm run build:mac   # or build:linux
```
...
|---------|--------------|--------|
| **2.1.6** | No hardcoded dev paths; one Customer Portal token; no personal greeting in UI (fixes #27); fix macOS Python crash (use bundled python3, raise recursion limit) | Current |
| 2.0.0 | Desktop app (Linux + macOS), browser UI, CLI parity, GitLab-only | Stable |
...
- **Version:** 2.1.6
- **Platforms:** Linux (AppImage x86_64 + ARM64) | macOS (DMG Intel + Apple Silicon)
