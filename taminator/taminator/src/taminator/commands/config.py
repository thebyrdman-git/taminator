...
- API tokens (JIRA, Portal, SupportShell; Portal token is also used for Hydra)
...
from ..core.auth_types import AuthType, TOKEN_REGISTRY, get_token_metadata
...
class ConfigManager:
    ...
    def show_current_config():
        ...
        for token_type in [AuthType.JIRA_TOKEN, AuthType.PORTAL_TOKEN, AuthType.SUPPORTSHELL_TOKEN]:
            
            token_name = get_token_metadata(token_type).name
        ...
        # Storage info
        keyring_line = "✅ Yes (recommended)" if KEYRING_AVAILABLE else "❌ No"
        keyring_note = "" if KEYRING_AVAILABLE else (
            "\n  Note: Keyring is optional. You can store tokens in the web UI (Settings) or in environment variables. "
            "To enable keyring: pip3 install keyring (macOS/Linux), or install python3-keyring from your package manager."
        )
        storage_info = f"""
Storage Information:
  Keyring Available: {keyring_line}{keyring_note}
...
"""
    ...
    def test_single_token(token_type: AuthType):
        """Test a single token."""
        console.print(f"\n🧪 Testing {get_token_metadata(token_type).name}...\n", style="cyan")
        ...
        if token_type == AuthType.JIRA_TOKEN:
            result = ConfigManager._test_jira_connection()
    ...
    @staticmethod
    def _test_jira_connection() -> bool:
        """Test JIRA connection (Red Hat or JIRA Cloud) using jira_config auth."""
        import requests
        base_url, auth_header, _ = jira_config.get_jira_auth()
        if not auth_header:
            console.print("  JIRA auth not configured (set token or, for Cloud, JIRA_EMAIL + JIRA_API_TOKEN)", style="red")
            return False
        api_url = jira_config.get_jira_api_url()
        try:
            response = requests.get(
                f"{api_url.rstrip('/')}/myself",
