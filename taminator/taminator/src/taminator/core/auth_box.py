...
class AuthBox:
    ...
    def get_token(self, token_type: AuthType, required: bool = True) -> Optional[str]:
        ...
        """
        # Try Vault first when VAULT_ADDR/VAULT_TOKEN are set (lazy import to avoid circular import)
        try:
            from taminator.core.hybrid_auth import get_token_for_auth_type
            token = get_token_for_auth_type(token_type, required=False)
            if token:
                return token
        except Exception:
            pass
        # Try keyring (most secure local)
        ...
        # Hydra = same credential as Portal; use Portal token if no separate Hydra token stored
        if not token and token_type == AuthType.HYDRA_TOKEN:
            token = self.get_token(AuthType.PORTAL_TOKEN, required=False)
    ...
    def _get_token_from_env(self, token_type: AuthType) -> Optional[str]:
        ...
        # Red Hat convention: Portal token is often set as PORTAL_TOKEN (no _API_)
        if token_type == AuthType.PORTAL_TOKEN and "PORTAL_TOKEN" not in env_names:
            env_names.insert(0, "PORTAL_TOKEN")
        for env_name in env_names:
            token = os.getenv(env_name)
            if token:
                return token
        # JIRA Cloud: env can provide JIRA_EMAIL + JIRA_API_TOKEN when JIRA_BASE_URL is *.atlassian.net
        if token_type == AuthType.JIRA_TOKEN:
            base = (os.getenv("JIRA_BASE_URL") or "").strip()
            if "atlassian.net" in base and os.getenv("JIRA_EMAIL") and os.getenv("JIRA_API_TOKEN"):
                return (os.getenv("JIRA_API_TOKEN") or "").strip() or None
        return None
    
    def _get_token_from_config(self, token_type: AuthType) -> Optional[str]:
        """Get token from ~/.config/taminator/ui_tokens.json (same file the web UI Settings uses).
        Supports encoded storage and legacy plain JSON. For JIRA_TOKEN, also accepts JIRA Cloud
        config (jira_base_url + jira_email + jira_api_token when base is *.atlassian.net)."""
        try:
            from pathlib import Path
            from taminator.core.token_store import load_ui_tokens
            config_path = Path.home() / ".config" / "taminator" / "ui_tokens.json"
            data = load_ui_tokens(config_path)
            key = token_type.value
            token = (data.get(key) or "").strip() or None
            if token:
                return token
            # JIRA Cloud: treat email + API token as having a token so @auth_required passes
            if token_type == AuthType.JIRA_TOKEN:
                base = (data.get("jira_base_url") or "").strip()
                if "atlassian.net" in base and data.get("jira_email") and data.get("jira_api_token"):
                    return (data.get("jira_api_token") or "").strip() or None
            return None
        except Exception:
            return None
    ...
    def _raise_token_missing_error(self, token_type: AuthType):
        ...
        from .auth_types import get_token_metadata
        metadata = get_token_metadata(token_type)
    ...
    def check_kerberos_ticket(self) -> AuthResult:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # We only check presence; expiry could be parsed from klist output if needed later.
