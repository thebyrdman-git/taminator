...
class TAMCallNotesPoster:
    ...
    def auto_link_cases_and_jira(self, content: str) -> str:
        ...
        # Link JIRA tickets (AAP-, AAPRFE-, ANSTRAT-) using configured base URL
        try:
            from taminator.core import jira_config
            def _jira_link(match):
                return f'[{match.group(1)}]({jira_config.get_jira_browse_url(match.group(1))})'
            jira_pattern = r'\b((?:AAP|AAPRFE|ANSTRAT)-\d+)\b'
            content = re.sub(jira_pattern, _jira_link, content)
        except Exception:
            jira_pattern = r'\b((?:AAP|AAPRFE|ANSTRAT)-\d+)\b'
            content = re.sub(jira_pattern, r'[\1](https://issues.redhat.com/browse/\1)', content)
...
- 📞 Contact your TAM to schedule calls
...
### Call Details
- **Duration**: 60 minutes
- **Attendees**: Red Hat TAM, [Customer attendees]
...
- [ ] Follow up on case 04123456 - Owner: TAM - Due: Next week
