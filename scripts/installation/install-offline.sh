#!/bin/bash
# RFE Automation Tool - OFFLINE Installation Script
# Pre-bundled with rhcase for offline/no-VPN testing
...
echo "🚀 RFE Automation Tool - Offline Installation"
echo "==============================================="
echo ""
...
main() {
    ...
    # Check for bundled rhcase
    RHCASE_DIR="./rhcase"
    
    if [ -d "$RHCASE_DIR" ]; then
        log_success "Found pre-bundled rhcase (offline mode)"
        log_info "Using bundled version (no VPN required)"
    else
        log_error "rhcase directory not found!"
        log_error "This is the OFFLINE installer - rhcase should be bundled"
        log_info "Use install-improved.sh if you have VPN access"
        exit 1
    fi
